export default (ctx, canvas, Listener) => {
    let functions =  {
        ...
        fillText: async ({ alpha, style, style2, text, x, y, add, font }, notCanvas) => {
            ...
            //ctx.globalAlpha = isNaN(Number(alpha)) ? Listener.gameState.alphaHUD : alpha
            for (let i in data) {
                if (font) ctx.font = (data[i].bold ? 'bold ' : '')+(data[i].italic ? 'italic ' : '')+font
                else ctx.font = font
            ...
            /*let oldAlpha = Number(String(ctx.globalAlpha))
            ctx.globalAlpha = isNaN(Number(alpha)) ? Listener.state.alphaHUD : alpha
            ctx.font = font
            if (add) {
                ctx.fillStyle = style2 || 'black'
